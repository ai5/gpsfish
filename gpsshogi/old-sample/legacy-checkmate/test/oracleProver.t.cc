...
class OracleProverTest : public CppUnit::TestFixture 
{
  CPPUNIT_TEST_SUITE(OracleProverTest);
...
CPPUNIT_TEST_SUITE_REGISTRATION(OracleProverTest);
...
static bool isCheckmate(const char *org, const char *similar)
{
  ...
  OracleProver<CheckHashTable> prover(table);
  Move best_move;
  const bool result = prover.proofWin(e_state2, path, record, best_move);
  if (! result)
  {
    CheckTableAnalyzer analyzer(table.getTwinTable());
    std::ofstream os("oracleProverTest.log");
    analyzer.showProofTree(record,e_state.getHash(),path,true,os);

    CheckmateRecorder::DepthTracer::maxVerboseLogDepth = 20;
    prover.proofWin(e_state2, path, record, best_move);
...
void OracleProverTest::testCheckLong()
{
...
void OracleProverTest::testInvalidDefense()
{
  ...
  OracleProver<CheckHashTable> prover(table);
  const CheckHashRecord *oracle = table.find(e_state.getHash());

  const bool result2 = prover.proofWin(e_state2, path, oracle, check_move);
  CPPUNIT_ASSERT_EQUAL(true, result2);
  CPPUNIT_ASSERT_EQUAL(m23KE, check_move);
#if 0
  // ImmediateCheckmate が対応していないので難しい
  ...
  const bool result3 = prover.proofWin(e_state3, path, oracle, check_move);
  ...
  const bool result4 = prover.proofWin(e_state4, path, oracle, check_move);
  CPPUNIT_ASSERT_EQUAL(false, result4);
}

void OracleProverTest::testInvalidDefenseLose()
{
  SimpleState state=CsaString(
    "P1 *  *  *  *  *  *  * -KE-OU\n"
    "P2 *  *  *  *  *  *  * -KA-KY\n"
  ...
  const bool result2 = prover.proofLose(e_state2, path, oracle, last_move);
  CPPUNIT_ASSERT_EQUAL(true, result2);
  ...
  const bool result3 = prover.proofLose(e_state3, path, oracle, last_move);
  CPPUNIT_ASSERT_EQUAL(true, result3);
  ...
  const bool result4 = prover.proofLose(e_state4, path, oracle, last_move);
  CPPUNIT_ASSERT_EQUAL(false, result4);
}

void OracleProverTest::testDistance()
{
  // bug 35
  SimpleState state=CsaString(
    "P1 *  *  *  *  *  *  * -OU * \n"
  ...
  const bool result = prover.proofWin(e_stateSimilar, pathSimilar,
				      oracle, checkmate_move2);
  CPPUNIT_ASSERT_EQUAL(check_move, checkmate_move2);
  CPPUNIT_ASSERT_EQUAL(true, result);

  const CheckHashRecord *recordSimilar = table.find(e_stateSimilar.getHash());
  CPPUNIT_ASSERT_EQUAL(distance, static_cast<int>(oracle->distance));
  CPPUNIT_ASSERT_EQUAL(distanceSimilar, static_cast<int>(recordSimilar->distance));
}

void OracleProverTest::testAdjustMove()
{
  const Move m1 = 
  ...
  Move checkmate_move2 = Move::INVALID();
  OracleProver<CheckHashTable> prover(table);
  const bool result = prover.proofWin(e_state2, path, oracle, checkmate_move2);
  CPPUNIT_ASSERT_EQUAL(true, result);
  ...
  
  {
    ...
    OracleProver<CheckHashTable> prover(table);
    const bool result = prover.proofWin(state2, path, oracle, checkmate_move2);
    CPPUNIT_ASSERT_EQUAL(true, result);
    CPPUNIT_ASSERT_EQUAL(checkmate_move.to(), checkmate_move2.to());
    CPPUNIT_ASSERT_EQUAL(checkmate_move.ptype(), checkmate_move2.ptype());
...
void OracleProverTest::testNoCheck()
{
  SimpleState state=CsaString(
    "P1 *  *  *  *  *  *  * -OU * \n"
    "P2 *  *  *  *  *  *  *  *  * \n"
  ...
  Move checkmate_move2 = Move::INVALID();
  OracleProver<CheckHashTable> prover(checker.getTable(BLACK));
  ProofOracleAttack<BLACK> oracle(record);
  const bool result = prover.proofWin(e_state2, path, record, checkmate_move2);
  CPPUNIT_ASSERT_EQUAL(false, result);
}



void OracleProverTest::testCheck()
{
  ...
  OracleProver<CheckHashTable> prover(table);
  const bool result = prover.proofWin(e_state, path, oracle, best_move);
  CPPUNIT_ASSERT_EQUAL(true, result);
  CPPUNIT_ASSERT_EQUAL(best_move, oracle->bestMove->move);
  ...
  CPPUNIT_ASSERT_EQUAL(true, prover.proofWin(e_state, path, oracle, best_move));
  CPPUNIT_ASSERT_EQUAL(best_move, oracle->bestMove->move);
  ...
  // 駒が足りないと詰まない
  {
    ...
    CPPUNIT_ASSERT_EQUAL(false, prover.proofWin(h2, path, oracle, best_move));
  }
  ...
  // 玉が動いても詰まない
  {
    ...
    const bool result = prover.proofWin(h2, path, oracle, best_move);
    CPPUNIT_ASSERT_EQUAL(false, result);
  }
  // 王手でも詰まない
  {
...
void OracleProverTest::testPawnCheckmate()
{
  SimpleState state=CsaString(
    "P1+NY+TO *  *  *  * -OU-KE-KY\n"
    "P2 *  *  *  *  * -GI-KI *  *\n"
  ...
  CPPUNIT_ASSERT_EQUAL(false, prover.proofWin(e_state, path, oracle, best_move));
}

void OracleProverTest::testPawnCheckmateLose()
{
  SimpleState state=CsaString(
    "P1 *  *  *  *  *  *  * -KE * \n"
    "P2 *  *  *  *  *  *  * -OU * \n"
  ...
  const CheckHashRecord *oracle = table.find(e_state.getHash());
  OracleProver<CheckHashTable> prover(table);
  CPPUNIT_ASSERT_EQUAL(true, prover.proofLose(e_state, path, oracle, last_move));
  ...
  CPPUNIT_ASSERT_EQUAL(false, prover.proofLose(e_stateNoEscape, path, oracle, last_move));
}

void OracleProverTest::testCheckAfterAttack()
{
  SimpleState state=CsaString(
    "P1+NY+TO *  *  *  * -OU-KE-KY\n"
    "P2+FU *  *  *  * -GI-KI *  *\n"
  ...
  // 詰む局面は詰む
  OracleProver<CheckHashTable> prover(table);
  CPPUNIT_ASSERT_EQUAL(true, prover.proofLose(e_state, path, oracle));
  ...
  // 玉が動くと同じ手順では詰まない
  {
    ...
    CPPUNIT_ASSERT_EQUAL(false, prover.proofLose(h2, path ,oracle));
  }
}
